...
def compute_summaries(
    ...
    for experience in valid_dataset:
        ...
        total_loss, outputs = model_net.compute_loss(
    ...
    error_list_dict = {
        k: np.array(v).mean() for k, v in error_list_dict.items()
    }
        ...
        with summary_writer.as_default():
            for name, loss in error_list_dict.items():
                tf.compat.v2.summary.scalar(
                    name="model_loss/" + name, data=loss, step=train_step
                )

    # generate video from sequence dataset
    valid_dataset = valid_dataset.flat_map(
    ...
    while episode < num_episodes_to_render:
        ...
        if experience.is_last():
            episode += 1
        prev_rew = tf.expand_dims(
            tf.expand_dims(experience.reward, axis=0), axis=0
        )
    ...
    reconstruct_images_from_latents = concat_images(
        reconstruct_images_from_latents, image_keys
    )

    all_images = tf.concat(
        [images, reconstruct_images, reconstruct_images_from_latents], axis=2
    )
...
def concat_images(images, image_keys):
    ...
    if type(images[0][0]) is OrderedDict:
        images = pad_and_concatenate_videos(
            images, image_keys=image_keys, is_dict=True
        )
    else:
        images = pad_and_concatenate_videos(
            images, image_keys=image_keys, is_dict=False
        )
...
def train_world_model(
    ...
    with tf.summary.record_if(
        ...
        tfrecords = [
            p
            for p in list(
                glob.glob(os.path.join(dataset_dir, "dataset.tfrecord.*"))
            )
            if "spec" not in p
        ]
        ...
        dataset = (
            example_encoding_dataset.load_tfrecord_dataset(
                tfrecords[: int(len(tfrecords) * 0.8)],
        ...
        checkpoint_manager = tf.train.CheckpointManager(
            checkpoint,
            os.path.join(root_dir, "checkpoint"),
            max_to_keep=5,
