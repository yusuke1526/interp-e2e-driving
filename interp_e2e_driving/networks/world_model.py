...
class VisionModel(tf.Module):
  ...
  def __init__(self, input_names, reconstruct_names, obs_size=64, base_depth=32, latent_size=64, decoder_stddev=np.sqrt(0.1, dtype=np.float32), name=None):
  ...
  def __call__(self, images):
    ...
    for name, decoder in self.decoders.items():
        reconstructs[name] = decoder(features[name]).mean()
    return features, reconstructs
...
if __name__ == '__main__':
  ...
  #   'base_depth': 32,
  #   'input_names': ['rgb'],
  #   'reconstruct_names': ['rgb'],
  #   'obs_size': 128
  # }
  # rgb = tf.zeros([batch_size, params['obs_size'], params['obs_size'], 3], tf.float32)
  # images = {'rgb': rgb}
  # vision = VisionModel(**params)
  # features, reconstructs = vision(images)
  # print(features)
  # print(reconstructs['rgb'].mean())
