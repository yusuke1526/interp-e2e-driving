...
class WorldModel(tf.Module):
    ...
    def compute_loss(
        ...
        next_z_means, next_z_log_vars, next_zs = self.vision.encode_sequence(
            next_images
        )
        ...
        (
            ...
            vision_loss,
        ) = self.vision.compute_sequence_loss(images)
        outputs.update(
            {
            ...
            outputs.update(
                {
                    ...
                    + name: tf.reduce_mean(reconstruction_errors[name]),
                }
            )
        loss = vision_loss
        ...
        images = tf.concat(
            [
                tf.image.convert_image_dtype(images[k], tf.float32)
                for k in list(set(self.input_names + self.reconstruct_names))
            ],
            axis=-2,
        )
        posterior_images = tf.concat(list(posterior_images.values()), axis=-2)
    ...
    def train(
        ...
        experience,
        weights=None,
        train_flag={"vision": True, "memory": False, "controller": False},
    ):
    ...
    def sample_z_sequence(self, images, actions, rewards, step_types):
        ...
        images = {
            name: image_sequence[:, :sequence_length]
